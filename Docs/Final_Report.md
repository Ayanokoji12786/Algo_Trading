...
A second full audit was run (every module re-read, all four CLIs + the mock-broker demo + the vendor smoke-test template executed end to end, all tests green). Findings and fixes below; **215 tests pass** (up from 202).

1. **Look-ahead bias via a negative execution delay (the most serious finding).** `next_execution_date(calendar, decision_date, delay)` computed `exec_pos = pos + delay` and returned `calendar[exec_pos]` with no guard against `exec_pos < 0`. A negative `execution_delay_sessions` therefore didn't error — Python's negative indexing silently wrapped around and returned a **future** calendar date (reproduced: decision `2020-01-01`, delay `-1` → "execution" `2020-05-19`), meaning trades would execute on prices that didn't exist at decision time. This is the exact failure the whole point-in-time architecture exists to prevent. Fixed at three layers: (a) `next_execution_date` now raises on any negative delay, protecting every caller (both engines, both paper traders, and any `SleeveSpec`) regardless of upstream validation; (b) `BacktestConfig.__post_init__` rejects a negative delay (and an invalid `rebalance_frequency`, which the plain engine otherwise treated silently as "weekly", and a non-positive `initial_capital`); (c) `SleeveSpec.__post_init__` rejects a negative delay, an invalid frequency, a negative `risk_share`, and a non-positive `participation_cap_fraction`. Regression test: `tests/unit/test_execution_delay_lookahead.py` (13 cases).

2. **Unused `pyyaml` dependency removed.** `pyyaml>=6.0` was declared in `pyproject.toml` but imported nowhere in the codebase — needless install weight and supply-chain surface. Removed; the remaining runtime deps (`pandas`, `numpy`, `scipy`, `pyarrow`) are all actually used.

**Re-verified from the first pass:** every fix from the first audit pass is still in place (path-traversal validation, IST-timezone epoch conversion, participation-cap exits, atomic cache writes, the no-silent-asset-class-fallback guards, drawdown-guardrail wiring, PIT defensive copy, config validation). All other computed-index accesses (`calendar[pos-1]`, `.iloc[-window:]`, etc.) were re-checked and are correctly guarded — the negative-delay path was the only unguarded one. Every user-facing feature was executed and confirmed working: `run_backtest`, `run_india_blended_backtest`, `run_robustness_suite` (DSR/PBO/Monte-Carlo artifacts all regenerated), the mock-broker paper-trading demo, and the vendor smoke-test template.
