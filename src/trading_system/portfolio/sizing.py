...
from trading_system.config.schema import RiskConfig, TrendStrategyConfig
...
def compute_target_weights(
    ...
    raw_risk_unit = {
        s.symbol: s.direction / max(s.annualized_vol, vol_floor_annualized)
        for s in usable
    }

    asset_classes = sorted({s.asset_class for s in usable})
    by_class: dict[str, list[Signal]] = {ac: [] for ac in asset_classes}
    for s in usable:
        by_class[s.asset_class].append(s)

    # Step 1+2: raw_risk_unit already inverse-vol-scales each instrument so
    # that its own risk contribution (raw_risk_unit * vol) equals its signal
    # strength |direction|, independent of the instrument's own volatility.
    # To equalize ex-ante risk *across* asset classes (not equal notional),
    # normalize each class's members by the class's total signal-weighted
    # risk (sum of |direction|, NOT sum of |raw_risk_unit| -- normalizing by
    # the latter would undo the inverse-vol scaling and make risk
    # contribution proportional to volatility, the opposite of the intent),
    # then split the total risk budget equally across asset classes that
    # actually have a live signal.
    ...
    for ac, members in by_class.items():
        ...
        if class_risk_gross == 0:
            continue
        for s in members:
            class_weight[s.symbol] = (
                raw_risk_unit[s.symbol] / class_risk_gross
            ) / n_classes
    ...
    estimated_portfolio_vol = math.sqrt(
        sum((w * vol_by_symbol[sym]) ** 2 for sym, w in class_weight.items())
    )
    if estimated_portfolio_vol == 0:
        return {}
    scale = risk_config.portfolio_vol_target_annualized / estimated_portfolio_vol
    scaled_weight = {sym: w * scale for sym, w in class_weight.items()}

    # Step 4: instrument and asset-class caps.
    capped = {
        sym: max(
            -risk_config.max_instrument_weight,
            min(risk_config.max_instrument_weight, w),
        )
        for sym, w in scaled_weight.items()
    }
    for ac, members in by_class.items():
        symbols = [s.symbol for s in members if s.symbol in capped]
